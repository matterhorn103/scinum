...
impl TryFrom<SciNum> for Decimal {
    ...
    /// Attempts to convert a `SciNum` into a `rust_decimal::Decimal`, dropping
    /// any uncertainty.
    ///
...
impl From<SciNum> for BigDecimal {
    /// Converts a `SciNum` into a `bigdecimal::BigDecimal`, dropping any uncertainty.
...
impl Neg for &SciNum {
    ...
    #[inline]
    fn neg(self) -> SciNum {
        SciNum { negative: !self.negative, ..*self }
    }
}
...
mod tests {
    ...
    fn new_largest_exponent() {
        let _n = SciNum::new(1, i16::MAX);
    }

    #[test]
    fn new_smallest_exponent() {
        let _n = SciNum::new(1, i16::MIN);
    }

    #[test]
    fn new_largest_significand() {
        let _n = SciNum::new(u64::MAX.into(), 0);
    }

    #[test]
    fn new_largest_negative_significand() {
        let _n = SciNum::new(-i128::from(u64::MAX), 0);
    }

    #[test]
    #[should_panic]
    fn new_invalid_significand() {
        let _n = SciNum::new(i128::from(u64::MAX) + 1, 0);
    }
    ...
    fn div_exact() {
        ...
        assert_eq!(
            SciNum::new(60, 0) / SciNum::new(30, 0),
            SciNum::new(2, 0),
        );
        // Non-recurring result with different exponent
        assert_eq!(
            SciNum::new(30, 0) / SciNum::new(60, 0),
            SciNum::new(5, -1),
        );
        // Identical recurring results with different pairs of starting numbers
        assert_eq!(
            SciNum::new(30, 0) / SciNum::new(60, 0),
            SciNum::new(3, 6) / SciNum::new(6, 6),
        );
        // Recurring results
        assert_eq!(
            (SciNum::new(1, 0) / SciNum::new(3, 0)),
            SciNum::new(3333333333333333333, -19),
        );
        // Fails as result is currently truncated rather than rounded
        //assert_eq!(
        //    (SciNum::new(1, 0) / SciNum::new(9, 0)),
        //    SciNum::new(1111111111111111112, -19),
        //);
    }
    ...
    #[test]
    fn inv() {
        assert_eq!(SciNum::new(4, 0).inv(), SciNum::new(25, -2));
        assert_eq!(SciNum::new(5, -1).inv(), SciNum::new(2, 0));
    }

    #[test]
    fn neg() {
        let n_pos = SciNum::new(4, 0);
        let n_neg = n_pos.neg();
        assert_eq!(n_neg, SciNum::new(-4, 0));
    ...
    fn display() {
        ...
        assert_eq!(SciNum::new(10000, 0).to_string(), "10000");
        assert_eq!(SciNum::new(1000, 0).to_string(), "1000");
        assert_eq!(SciNum::new(100, 0).to_string(), "100");
        assert_eq!(SciNum::new(10, 0).to_string(), "10");
        assert_eq!(SciNum::new(1, 0).to_string(), "1");
        assert_eq!(SciNum::new(1, -1).to_string(), "0.1");
        assert_eq!(SciNum::new(1, -2).to_string(), "0.01");
        assert_eq!(SciNum::new(1, -3).to_string(), "0.001");
        assert_eq!(SciNum::new(1, -4).to_string(), "0.0001");
        assert_eq!(SciNum::new(1, -5).to_string(), "0.00001");
        assert_eq!(SciNum::from(dec!(0.00001)).to_string(), "0.00001");
        assert_eq!(SciNum::new(325, -4).to_string(), "0.0325");
        assert_eq!(SciNum::new(-325, -4).to_string(), "-0.0325");
        dbg!("Reached here");
        assert_eq!(SciNum::new(85130, -3).to_string(), "85.130");
        //dbg!(sci!(25691.29854));
        ...
        assert_eq!(SciNum::new(325, -6).to_string(), "3.25e-4"); // Not 0.000325
        assert_eq!(SciNum::new(-325, -6).to_string(), "-3.25e-4");
        assert_eq!(SciNum::new(8174036, 0).to_string(), "8.174036e6");
        ...
        // 81700 with 3 sf = 8.17e4 = (817, 2) => "8.17e4"
        assert_eq!(SciNum::new(817, 2).to_string(), "8.17e4");
        // 81700 with 5 sf = 8.1700e4 = (81700, 0) => "81700"
        assert_eq!(SciNum::new(81700, 0).to_string(), "81700");
    ...
    fn from_str() {
        ...
        // Decimal
        assert_eq!(SciNum::from_str("0.0859").unwrap(), SciNum::new(859, -4));
        // Decimal without integral part before decimal point
        assert_eq!(SciNum::from_str(".0859").unwrap(), SciNum::new(859, -4));
        // Negative decimal
        ...
        // Large exponents
        assert_eq!(SciNum::from_str("1.5e18").unwrap(), SciNum::new(15, 17));
        assert_eq!(
        ...
        assert_eq!(SciNum::from_str("0.5293040185492948").unwrap(), SciNum::new(5293040185492948, -16));
        // Excess precision should be silently truncated to 16 sf
        // TODO: maybe in future should be rounded rather than truncated?
        assert_eq!(SciNum::from_str("0.529304018549294841").unwrap(), SciNum::new(5293040185492948, -16));
        // Make sure incorrectly formatted strings fail
        assert!(SciNum::from_str("not a number").is_err());
        assert!(SciNum::from_str("x.482").is_err());
        assert!(SciNum::from_str("52.x").is_err());
        assert!(SciNum::from_str("-2.42F-4").is_err());
    }
